...
Este capítulo apresenta os conceitos teóricos que fundamentam a solução proposta neste trabalho. Inicialmente, são discutidos mHealth, planos de cuidado, plataformas no-code e letramento em saúde, de modo a situar o contexto de aplicação da pesquisa. Em seguida, são abordados os LLMs, seus usos em saúde, o papel dos guardrails, a linguagem simples, a simplificação textual e a adaptação ao perfil do paciente. Por fim, o capítulo discute os riscos da simplificação textual com LLMs em saúde e apresenta o conceito de legibilidade textual, incluindo o índice Flesch-PT, utilizado posteriormente como indicador automático na avaliação da solução.

\section{mHealth e saúde digital}
...
Neste trabalho, mHealth é tratado como o contexto de aplicação da solução proposta. O foco não está em desenvolver uma nova categoria de aplicação móvel em saúde, mas em investigar como LLMs podem apoiar a simplificação e adaptação linguística de textos técnicos de saúde que poderiam ser apresentados em uma aplicação mHealth.

\subsection{Planos de cuidado e plataformas no-code em mHealth}
...
\subsection{Letramento em saúde}
...
A relação entre letramento em saúde e adesão ao tratamento é discutida na literatura, mas deve ser compreendida como multifatorial. \citeonline{miller2016healthliteracy} identificou associação positiva entre letramento em saúde e adesão em condições crônicas e agudas. \citeonline{hyvert2023healthliteracy} também discutem essa relação no contexto da adesão medicamentosa em doenças crônicas, destacando que ela é influenciada por múltiplos elementos relacionados ao paciente, ao tratamento e ao contexto de cuidado.
...
Guardrails podem ser definidos como mecanismos de controle, restrição ou validação aplicados ao uso de LLMs. Seu objetivo é reduzir a probabilidade de comportamentos indesejados, como respostas fora do escopo e geração de informações incorretas. \citeonline{dong2024guardrails} descrevem guardrails como tecnologias de salvaguarda que podem atuar sobre entradas e saídas de LLMs, considerando os riscos e requisitos específicos de cada aplicação.
...
\subsection{Simplificação textual e adaptação ao perfil do paciente}
...
\subsection{Riscos da simplificação textual com LLMs em saúde}
...
A avaliação da simplificação textual pode ser apoiada por indicadores automáticos de legibilidade, que buscam estimar propriedades linguísticas de um texto, como o comprimento médio das frases e o número médio de sílabas por palavra. O Índice de Legibilidade de Flesch, em sua formulação original para o inglês, é dado por:

\[
  \text{Flesch} = 206{,}835 - 1{,}015 \times \overline{\text{CP}} - 84{,}6 \times \overline{\text{SP}}
\]

onde $\overline{\text{CP}}$ é o número médio de palavras por sentença e $\overline{\text{SP}}$ é o número médio de sílabas por palavra. Valores mais altos indicam menor complexidade linguística.

A constante 206,835 foi calibrada empiricamente para o inglês. \citeonline{martins1996readability} observaram que o português brasileiro apresenta, em média, maior número de sílabas por palavra do que o inglês, em decorrência de sua morfologia mais rica e de palavras tipicamente mais longas. Aplicada diretamente a textos em português, a fórmula original produziria pontuações sistematicamente inferiores às de textos ingleses de complexidade equivalente, tornando as faixas de classificação originais inadequadas. Para corrigir esse viés, os autores propõem acrescentar 42 pontos à constante da fórmula, obtendo a adaptação para o português brasileiro:

\[
  \text{Flesch-PT} = 248{,}835 - 1{,}015 \times \overline{\text{CP}} - 84{,}6 \times \overline{\text{SP}}
\]

Com esse ajuste, textos em português com nível de complexidade equivalente ao de textos em inglês tendem a produzir pontuações comparáveis, permitindo o uso das mesmas faixas de classificação. A Tabela~\ref{tab:classificacao-flesch-pt} apresenta a escala adotada neste trabalho.

\begin{table}[htbp]
  \centering
  \caption{Escala de classificação do índice Flesch-PT \cite{martins1996readability}.}
  \label{tab:classificacao-flesch-pt}
  \small
  \renewcommand{\arraystretch}{1.15}
  \begin{tabular}{cc}
    \hline
    \textbf{Índice Flesch-PT} & \textbf{Classificação} \\
    \hline
    75 a 100 & Muito fácil \\
    50 a 74  & Fácil \\
    25 a 49  & Pouco difícil \\
    0 a 24   & Muito difícil \\
    \hline
  \end{tabular}

  {\footnotesize Fonte: \citeonline{martins1996readability}.}
\end{table}

No contexto deste trabalho, o Flesch-PT auxilia na comparação entre o texto original e o texto simplificado, indicando se a versão gerada apresenta sinais de maior legibilidade.
