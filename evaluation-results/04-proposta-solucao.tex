...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \draw[arrow] (instrucao.south) |- (geracaoTexto.north);
    \draw[arrow] (instrucao.south) |- (geracaoFAQ.north);
...
Além do conteúdo textual, a solução utiliza informações do perfil comunicacional do paciente. Esse perfil orienta a forma da adaptação produzida pela LLM, considerando características como idade, grupo de condição e formação educacional. Esses dados são utilizados para ajustar vocabulário, extensão das frases e grau de explicação dos termos técnicos, sem modificar a orientação clínica presente no texto original. 
...
Em aplicações de saúde, esse cuidado é particularmente relevante, pois a utilização de IA generativa exige atenção adicional à factualidade, à segurança e à confiabilidade das informações produzidas \cite{gangavarapu2024healthcare}. No contexto deste trabalho, isso implica estabelecer delimitações claras sobre o papel do modelo e sobre os limites da resposta produzida. Ainda assim, os guardrails não devem ser entendidos como garantia absoluta de correção: eles buscam reduzir riscos, mas não eliminam completamente a possibilidade de erro, interpretação inadequada ou perda de informação relevante. Por essa razão, a solução deve ser compreendida como apoio à comunicação em saúde, e não como substituta de revisão profissional ou validação clínica.
...
Entretanto, o Flesch-PT deve ser interpretado com cautela. O índice avalia aspectos formais do texto, mas não mede diretamente a compreensão real do paciente, nem garante que o conteúdo médico tenha sido preservado corretamente. Um texto pode apresentar melhor legibilidade estimada e, ainda assim, não ser adequado se perder informações relevantes, alterar o sentido do conteúdo original ou não atender aos guardrails definidos na solução. 

Essa separação é importante para evitar que a legibilidade seja confundida com segurança clínica. Um texto mais simples pode ser melhor para comunicação, mas somente será adequado se preservar o conteúdo relevante do original. Assim, a proposta combina dois eixos de avaliação: de um lado, o Flesch-PT como indicador automático de legibilidade; de outro, critérios de aderência aos guardrails e preservação do significado do texto original. 

\section{Delimitações da solução} \label{sec:delimitacoes-solucao} 
...
Outra delimitação está relacionada ao uso de LLMs. Embora os guardrails reduzam riscos de geração de informações inadequadas ou externas ao conteúdo fornecido, eles não garantem ausência total de erros, nem substituem a revisão por profissionais qualificados. Por esse motivo, a solução é delimitada como ferramenta de apoio à comunicação em saúde, e não como mecanismo autônomo de validação clínica. 

A privacidade de dados não constitui eixo de investigação deste trabalho. A solução foi construída e demonstrada em ambiente controlado, com uso de perfis simulados, sem envolver dados reais de pacientes. Assim, aspectos como conformidade legal, anonimização, consentimento, armazenamento seguro de dados sensíveis e políticas de governança ficam fora do escopo deste TCC, podendo ser considerados em trabalhos futuros ou em uma eventual integração da solução a um ambiente real de uso. 

Com essas delimitações, a proposta se posiciona como uma integração aplicada de elementos discutidos ao longo do trabalho: LLMs para comunicação com pacientes, simplificação textual em saúde, adaptação comunicacional ao perfil do usuário, uso de guardrails e avaliação por indicador automático de legibilidade. A contribuição está em reunir esses elementos em uma implementação funcional voltada a um cenário mHealth baseado em planos de cuidado, tomando a Takere como ambiente de referência para aplicação da abordagem.