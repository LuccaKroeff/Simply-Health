...
A avaliação foi planejada considerando três execuções para cada combinação entre material e perfil, resultando em 18 execuções principais: dois materiais, três perfis simulados e três execuções independentes para cada combinação. Cada execução correspondeu ao processamento de um material para um perfil específico, podendo envolver uma ou mais tentativas internas de geração quando a saída era rejeitada pelos guardrails. Todas as execuções utilizaram o modelo \texttt{gemini-2.5-flash-lite} tanto para a simplificação textual quanto para o guardrail de fidelidade, conforme a configuração descrita na Tabela~\ref{tab:modelos-llm}.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{Xc}
    ...
    Aprovados na quarta tentativa\tablefootnote{Corresponde ao re-run realizado após o fallback da execução estomia-Ana-R2. As três tentativas internas dessa execução foram rejeitadas, ativando o fallback; o re-run subsequente foi aprovado na primeira tentativa, totalizando quatro tentativas no conjunto da sequência.} & 1/18 \\
