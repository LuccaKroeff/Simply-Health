...
Este capítulo apresenta os resultados obtidos a partir da avaliação da SimplyHealth. Inicialmente, são analisados os resultados de legibilidade textual, com base na comparação entre os valores de Flesch-PT dos textos originais e das versões simplificadas. Em seguida, são discutidos os resultados relacionados à preservação das informações essenciais, considerando a atuação dos guardrails e a análise preliminar das saídas aprovadas. O capítulo também apresenta exemplos qualitativos das simplificações geradas, uma síntese dos principais achados e uma discussão sobre o significado desses resultados no contexto da proposta.

\section{Resultados de legibilidade textual}
\label{sec:resultados-legibilidade-textual}

A avaliação de legibilidade foi realizada a partir da comparação entre o Flesch-PT dos textos originais e o Flesch-PT das versões simplificadas aprovadas para cada perfil. O índice foi calculado exclusivamente sobre o texto simplificado, excluindo o glossário e as perguntas frequentes, que são gerados como saídas complementares separadas. Os resultados consideram as 18 execuções principais da avaliação, todas com texto final aprovado e Flesch-PT computado. As tentativas internas rejeitadas pelos guardrails não foram consideradas no cálculo de legibilidade, pois não correspondem às versões finais apresentadas pela solução.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccccc}
    ...
    \hline
    Ana    & 3 & 41,8 & 48,8 & +7,0  & Pouco difícil \\
    Carlos & 3 & 41,8 & 59,0 & +17,2 & Fácil \\
    Maria  & 3 & 41,8 & 67,4 & +25,6 & Fácil \\
    \hline
  \end{tabular}
...
No material sobre estomia, todos os perfis apresentaram aumento médio no Flesch-PT após a simplificação. O menor ganho ocorreu no perfil de Ana, com aumento médio de +7,0 pontos, enquanto o maior ganho ocorreu no perfil de Maria, com aumento médio de +25,6 pontos. Carlos apresentou resultado intermediário, com ganho médio de +17,2 pontos. Esse padrão indica que a solução aplicou uma simplificação mais intensa para o perfil com menor escolaridade e menor familiaridade com linguagem técnica, mantendo uma adaptação mais moderada para o perfil com formação na área da saúde.

A diferença entre os perfis também se refletiu na classificação média dos textos. O texto original foi classificado como ``Pouco difícil'' pelo Flesch-PT. As versões geradas para Carlos e Maria atingiram a classificação ``Fácil'', enquanto as versões geradas para Ana permaneceram em ``Pouco difícil'', a mesma categoria do texto original. Isso sugere que a adaptação ao perfil de menor escolaridade e ao perfil intermediário produziu impacto mais expressivo na superfície linguística do texto.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccccc}
    ...
    \textbf{Perfil} & \textbf{Casos} & \textbf{Original} & \textbf{Simplif.} & \textbf{Ganho} & \textbf{Classif.} \\
    \hline
    Ana    & 3 & 51,8 & 49,5 & -2,3 & Pouco difícil \\
    Carlos & 3 & 51,8 & 56,6 & +4,8   & Fácil \\
    Maria  & 3 & 51,8 & 68,1 & +16,3  & Fácil \\
    \hline
  \end{tabular}
...
No material sobre diabetes, o mesmo gradiente entre perfis foi observado, embora com uma particularidade no perfil de Ana. Para esse perfil, a média das execuções apresentou variação negativa de -2,3 pontos. O texto original do material sobre diabetes foi classificado como ``Fácil'' pelo Flesch-PT, e as versões simplificadas para esse perfil resultaram em média ligeiramente abaixo desse valor, apresentando classificação ``Pouco difícil'' em duas das três execuções. Esse resultado não deve ser interpretado diretamente como falha da solução, pois Ana representa um perfil com ensino superior e familiaridade com a área da saúde. Assim, a manutenção de terminologia técnica e de uma estrutura textual mais próxima do original é coerente com a proposta de adaptação comunicacional ao perfil.

Carlos apresentou ganho médio de +4,8 pontos no material sobre diabetes, sugerindo uma simplificação moderada. Maria, por sua vez, apresentou ganho médio de +16,3 pontos, permanecendo na classificação ``Fácil'', assim como o texto original. Esse comportamento reforça o padrão observado no material sobre estomia: quanto menor a escolaridade e a familiaridade esperada com linguagem técnica de saúde, maior foi a intensidade da simplificação linguística produzida pela solução.

Considerando os 18 textos aprovados e com Flesch-PT computado, a média dos textos originais foi de 46,8, enquanto a média das versões simplificadas foi de 58,2. O ganho médio geral foi, portanto, de +11,4 pontos. Em 15 dos 18 casos houve aumento do Flesch-PT, enquanto 3 casos apresentaram ganho negativo. Todos os casos com ganho negativo ocorreram no par D-Ana, isto é, no material sobre diabetes para o perfil de Ana.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{Xccc}
    ...
    \midrule
    Flesch-PT original médio & 51,8 & 41,8 & 46,8 \\
    Flesch-PT simplificado médio & 58,1 & 58,4 & 58,2 \\
    Ganho médio & +6,3 & +16,6 & +11,4 \\
    Maior ganho observado & +20,3 & +26,5 & +26,5 \\
    Menor ganho observado & -3,8 & +5,4 & -3,8 \\
    Casos com ganho positivo & 6/9 & 9/9 & 15/18 \\
    Casos com ganho nulo ou negativo & 3/9 & 0/9 & 3/18 \\
    Casos com melhora de classificação & 0/9 & 7/9 & 7/18 \\
    Casos com regressão de classificação & 2/9 & 0/9 & 2/18 \\
    \bottomrule
  \end{tabularx}
...
Os resultados reforçam o gradiente observado nas tabelas anteriores: Ana apresentou os menores ganhos, Carlos ocupou uma posição intermediária e Maria apresentou os maiores ganhos. O caso D-Ana, com variações negativas nas três execuções, deve ser interpretado com cautela: o texto original do material sobre diabetes foi classificado como ``Fácil'', e as versões simplificadas resultaram em média ligeiramente abaixo desse valor. Esse comportamento pode decorrer da manutenção de linguagem técnica coerente com o perfil de formação na área da saúde, e evidencia os limites do Flesch-PT como único indicador de qualidade textual.
...
Além do texto simplificado, a solução também gerou perguntas frequentes baseadas no material original. Essas saídas passam pelo mesmo guardrail de fidelidade utilizado na simplificação, mas os eventos de rejeição correspondentes não foram contabilizados separadamente na avaliação. O índice de legibilidade também não é calculado para as perguntas frequentes. Portanto, os resultados quantitativos das seções anteriores referem-se exclusivamente ao texto simplificado. A análise a seguir é qualitativa e busca observar se a adaptação ao perfil ocorreu também nessas saídas.
...
A avaliação mostrou que a aplicação aumentou a legibilidade textual em 15 dos 18 textos aprovados com Flesch-PT computado. A média dos textos originais foi de 46,8, enquanto a média das versões simplificadas foi de 58,2, resultando em ganho médio geral de +11,4 pontos. Ao separar os materiais, o ganho médio foi de +6,3 no material sobre diabetes e de +16,6 no material sobre estomia.

Os resultados também mostraram diferenças entre os perfis simulados. Maria apresentou os maiores ganhos médios nos dois materiais, com +25,6 no material sobre estomia e +16,3 no material sobre diabetes. Carlos ocupou uma posição intermediária, com ganhos médios de +17,2 e +4,8, respectivamente. Ana apresentou os menores ganhos, com +7,0 no material sobre estomia e -2,3 no material sobre diabetes. Mudanças de classificação ocorreram em 9 dos 18 casos: no material sobre estomia, 7 versões passaram da classificação ``Pouco difícil'' para ``Fácil''; no material sobre diabetes, 2 versões do par D-Ana regrediram de ``Fácil'' para ``Pouco difícil''.
...
Com relação à pergunta de pesquisa, os achados sugerem que LLMs podem ser utilizados para reduzir o nível de leitura exigido por textos técnicos de saúde e adaptá-los ao perfil comunicacional de pacientes quando a geração é baseada em materiais previamente definidos, o perfil é usado apenas para orientar a comunicação e há restrições explícitas sobre o que o modelo pode ou não modificar. Dentro do escopo avaliado, a SimplyHealth demonstrou que LLMs podem apoiar a reescrita de conteúdos técnicos de saúde de modo sensível a diferentes perfis, desde que combinadas com prompts bem delimitados, guardrails de verificação e análise da preservação das informações essenciais. Neste trabalho, essa preservação deve ser compreendida como a manutenção do sentido e das orientações presentes no material original, e não como validação clínica autônoma do conteúdo.
...
Apesar dos resultados positivos, a avaliação reforça a necessidade de revisão humana especializada antes de qualquer uso em contexto real. A leitura manual realizada foi preliminar, os perfis utilizados foram simulados e o Flesch-PT não mede compreensão real por parte dos pacientes. Assim, os achados apoiam a viabilidade da abordagem, mas não permitem afirmar que os textos gerados estão clinicamente validados, que pacientes reais compreenderiam melhor os conteúdos ou que a abordagem melhora adesão ao tratamento ou desfechos de saúde. O capítulo seguinte retoma essas questões a partir das limitações e ameaças à validade do trabalho.
